...
function custom_post_type () {
	$label= array(
		...
		'not_found_in_trash' => __('No Hotel Found in the Trash'),
		'parent_item_colon' => '',
		'menu_name' => 'Hotels'
	);
	$args=array(
...
function my_taxonomies_hotel () {
	$label= array(
		...
		'all_items' => __('All Hotels Categories'),
...
function add_my_post_types_to_query( $query ) {
    if ( is_home() && $query->is_main_query() )
        $query->set( 'post_type', array( 'post', 'hotel', 'multiple_product' ) );
    return $query;
...
class wpb_widget extends WP_Widget {
	...
	public function widget ($args , $instance) {
		...
		echo __('Hello World! this is the custom widget.', 'wpb_widget_domain');
...
function shortcode_shortcode(){
	
	return '<span class="shortcode">Shortcode:This is the custom shortcode.</span>';
}
...
function custom_meta_box_markup(){
	...
	<div>
		...
		<input type="text" name="meta_box_text" value="<?php echo get_post_meta($post->ID,'meta_box_text',true); ?>">
	</div>
	<?php
}
...
function my_menu_pages(){
    add_menu_page('My Page Title', 'My Settings', 'manage_options', 'my_menu', 'my_menu_output' ,null, 25);
    add_submenu_page('my_menu', 'Submenu Page Title', 'Submenu page 1', 'manage_options', 'my_menu' );
    /*add_submenu_page('my_menu', 'Submenu Page Title2', 'Submenu page 2', 'manage_options', 'my_menu2' );*/
}
function my_menu_output(){
	...
	echo '<h2>'.$options['banner_heading'].'</h2>';
	echo '<img src="'.$options['logo'].'" alt="logo" style="width:150px;height:150px;" />';
	return $options['logo'];
}	
}
...
function banner_heading_setting(){
	$options = get_option('plugin_options');
	echo "<input type='text' name='banner_heading'  />";
...
function validate_setting($plugin_options) {
	if ($_POST) {
		$options=get_option('plugin_options');
		$plugin_options['banner_heading']=$_POST['banner_heading'];
	}
...
function themeslug_enqueue_style() {
    wp_enqueue_style( 'stylecustom', get_template_directory_uri() .'/stylecustom.css', false );
}
...
function my_script_enqueuer() {
   wp_register_script( "my_voter_script", get_template_directory_uri() .'/my_voter_script.js', array('jquery') );
...
add_action( 'init', 'my_ajax_profile' );

function my_ajax_profile() {
   wp_register_script( "my_profile_script", get_template_directory_uri() .'/my_profile_script.js', array('jquery') );
   wp_localize_script( 'my_profile_script', 'myAjax', array( 'ajaxurl' => admin_url( 'admin-ajax.php' )));        

   wp_enqueue_script( 'jquery' );
   wp_enqueue_script( 'my_profile_script' );

}
  add_action( 'wp_ajax_ajax_add', 'ajax_add' );
    add_action( 'wp_ajax_nopriv_ajax_add', 'my_login' );

    function ajax_add(){
    	global $wpdb,$user_ID;
 $user_login = esc_attr( get_the_author_meta( 'user_login', $user_ID ) );
        ...
        if(0 === count($errors)) 
         {  
           ...
           if (isset($_POST['password'])) {
             $wpdb->query( $wpdb->prepare( "UPDATE {$wpdb->signups} SET display_name = %s,user_password = %s,user_email = %s WHERE ID = %s", $_POST['username'],$_POST['password'],$_POST['email'], $user_ID ) );  
           }
            
            $wpdb->query( $wpdb->prepare( "UPDATE wp_users SET display_name = %s,user_email = %s WHERE ID = %s", $username,$email, $user_ID ) );  
            
   
   
            $success = 1; 
            echo '<div class="container col-md-4">'; 
            echo "<div class='alert alert-success'>Your Data is Successfully Added</div>";
            echo "</div>";
            ...
            //echo "<script type='text/javascript'>window.location.href='".home_url()."'</script>"; 
   			...
   			$data['message']='Your Data is Successfully Added';
        }  else{
        	$data['errors']=$errors;
        }
...
add_action( 'init', 'my_ajax_crud' );

function my_ajax_crud() {
   wp_register_script( "crud", get_template_directory_uri() .'/crud.js', array('jquery') );
   wp_localize_script( 'crud', 'myAjaxd', array( 'ajaxurl' => admin_url( 'admin-ajax.php' )));        

   wp_enqueue_script( 'jquery' );
   wp_enqueue_script( 'crud' );

}
  add_action( 'wp_ajax_ajax_crud', 'ajax_crud' );
    add_action( 'wp_ajax_nopriv_ajax_crud', 'crud_login' );
      ...
      $errors['email'] = "Please enter the valid email.";
        ...
        if(0 === count($errors)) 
         {  
            ...
            $sql= "INSERT INTO wp_crudq(fname,lname,email) VALUES ('$fname','$lname','$email')" ;
...
function crud_login() {
 ...
 add_action( 'wp_ajax_crud_delete', 'crud_delete' );
    add_action( 'wp_ajax_nopriv_crud_delete', 'crudd_login' );
    function crud_delete(){
    	if (isset($_POST['id'])) {
    		$idd=$_POST['id'];
    		$db=mysqli_connect('localhost','root','', 'wordpress');
            $sql= "DELETE FROM wp_crudq WHERE id='$idd'" ;
            $res=mysqli_query($db,$sql); 
    	}
...
function woo_custom_product_searchform( $form ) {
    ...
    $form = '<form role="search" class="search" method="get" id="searchform" action="' . esc_url( home_url( '/'  ) ) . '">
		<div class="insearch">
			<label class="screen-reader-text" for="s">' . __( 'Search for:', 'woocommerce' ) . '</label>
			<input type="text" value="' . get_search_query() . '" name="s" id="s" placeholder="' . __( 'My Search form', 'woocommerce' ) . '" required/>
...
$product_ids = implode( ",", $viewed_products );
	...
	if ( $r->have_posts() ) {
		...
		while ( $r->have_posts()) {
			...
			$content = do_shortcode("[products ids='$product_ids']");
		}
...
function add_content_after_buynow() {
    ...
    add_action( 'woocommerce_after_shop_loop_item', 'add_content_after_buynow' );
...
function woocommerce_custom_fields_display(){
	...
	if ($custom_fields_woocommerce_title) {
		printf('<div><label style="color: green;">%s</label></div>',
